...
const colorMap = {
  'white_red': '#FFCCCB', // light red for "10"
  'light_orange': '#FFBE7D',
  'red': '#FF0000',
  'orange': '#FFA500',
  'yellow': '#FFFF00',
...
const BlocksArea = forwardRef(({ number, numberData, activeColor }, ref) => {
  ...
  // Add blocks method for API
  const addBlocks = (count, color) => {
    setBlocks(prevBlocks => {
      const newBlocks = [...prevBlocks];
      const blockColor = color || activeColor || 'red';
      const colorName = Object.keys(colorMap).find(key => 
        colorMap[key].toLowerCase() === blockColor.toLowerCase()
      ) || 'red';
  ...
  // Get clubs and examples from the JSON data
  const clubs = numberData[number]?.clubs;
  const examples = numberData[number]?.real_world_examples;

  return (
    <div className="blocks-area">
      {clubs && <ClubButtons clubs={clubs} onArrange={arrangeBlocks} />}
      ...
      {examples && <ExamplesPanel examples={examples} />}
    </div>
  );
});
